...
public:
	...
	bool my_is_start_label(const string& label) const {
		if (label.length() != 1)
			return false;
		return label[0] == 'b' || label[0] == 'B';
	}

	bool my_is_contiune_label(const string& label, int distance) const {
		if (label.length() != 1)
			return false;
		if (distance == 0)
			return true;
		return label[0] == 'i' || label[0] == 'I';
	}

	void MiningEvaluate(const vector<string>& resulted_labels, Metric& eval) const {
		static int idx, idy, endpos;
		unordered_set<string> golds;
		idx = 0;
		while (idx < labels.size()) {
			if (my_is_start_label(labels[idx])) {
				idy = idx;
				endpos = -1;
				while (idy < labels.size()) {
					if (!my_is_contiune_label(labels[idy], idy - idx)){
						endpos = idy - 1;
						break;
					}
					endpos = idy;
		...
		while (idx < resulted_labels.size()) {
			if (my_is_start_label(resulted_labels[idx])) {
				idy = idx;
				endpos = -1;
				while (idy < resulted_labels.size()) {
					if (!my_is_contiune_label(resulted_labels[idy], idy - idx)){
						endpos = idy - 1;
						break;
					}
					endpos = idy;
